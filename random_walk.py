...
"""
    ...
    def _e(self):
        ...
        while True:
            t = m2 / 2 + sum(
                [prob * jump * jump * _aux2(e * mu * jump) for jump, prob in self.jumps]
            )
            g = 1 + e * t
            ...
            tp = sum(
                [
                    prob * jump * jump * jump * mu * _paux2(e * mu * jump)
                    for jump, prob in self.jumps
                ]
            )
            gp = e * tp + t
            e -= g / gp
    ...
    def characteristics(self):
        ...
        E = (
            ...
            * a
            * b
            * (b / 2 - a / 2 + b * _aux2(h * b) - a * _aux2(h * a))
            / (b - a + b * _aux(h * b) - a * _aux(h * a))
        )
        return prob_H1, E
